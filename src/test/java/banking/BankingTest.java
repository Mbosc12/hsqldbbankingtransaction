...
public class BankingTest {
    ...
    @Test
    public void SoldeNegatifApresTransaction() throws SQLException {
        float avant = myDAO.balanceForCustomer(0);
        float av = myDAO.balanceForCustomer(1);
        try {
            myDAO.bankTransferTransaction(0, 1, myDAO.balanceForCustomer(0) + 20f);
            fail("Exception");
        } catch (Exception e) {

        }
        // Rien ne doit avoir changé
        assertEquals("Impossible!", avant, myDAO.balanceForCustomer(0), 0.001f);
        assertEquals("Impossible!", av, myDAO.balanceForCustomer(1), 0.001f);
    }

    @Test
    public void DebiteurInconnu() throws SQLException {
        float avant = myDAO.balanceForCustomer(1);
        try {
            myDAO.bankTransferTransaction(55, 1, 20f);
            fail("Exception");
        } catch (Exception e) {

        }
        assertEquals("Incorrecte!", avant, myDAO.balanceForCustomer(1), 0.001f);
    }

    @Test
    public void CrediteurInconnu() throws SQLException {
        float avant = myDAO.balanceForCustomer(1);
        try {   
            myDAO.bankTransferTransaction(1, 55, 20f);
